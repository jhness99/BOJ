...
/*
* 1.아이디어
* dp[i][j]는 i번째 행에 j위치에 사자가 있을경우 경우의 수라는 뜻이다
* 이때 j가 0이면 두곳다 비어있다는 뜻이다.
* 만약 2번째 행의 경우 0일경우, 1번째 행이 0,1,2인 경우 모두 더한 값이 0일경우의 값이다.
* 1일경우, 1번째 행이 1인경우는 불가능하기때문에 0,2 경우를 더한값이 1일경우의 값이다.
* 마찬가지로 2일경우, 1번행이 2인경우는 불가능하기 때문에 0, 1 경우를 더한값이 2일경우의 값이다.
*
* 이를 보면 점화식은 0일경우는 전 행의 0,1,2의 경우의 합, 1일경우는 전 행의 0,2, 2일경우는 전 행의 1,2 인 경우이다.
* 이를 점화식으로 표현하면
* dp[i][0] = dp[i - 1][0] + dp[i - 1][1] + dp[i - 1][2];
* dp[i][1] = dp[i - 1][0] + dp[i - 1][2];
* dp[i][2] = dp[i - 1][0] + dp[i - 1][1];
